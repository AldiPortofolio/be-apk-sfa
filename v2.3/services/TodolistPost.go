...
import (
	...
	"ottosfa-api-apk/hosts/acquitisions"
...
// TodolistPost ..
func (svc *Service) TodolistPost(bearer string, req models.TodolistPostV23Req, res *models.Response) {
	...
	//	if req.Label[0] != "" {
	//		acquitisionsRes, err := acquitisions.Send(req.Acquitisions, svc.General.SpanId)
	//		if err != nil || acquitisionsRes.Meta.Status == false {
	//			res.Meta = utils.GetMetaResponse("todolist.acquitision.failed")
	//			return
	//		}
	//
	//		_, err = redis.GetRedisKey("SFA:ACQUITISIONS:CHECK:PhoneNumber:"+ req.Acquitisions.PhoneNumber)
	//		if err != nil {
	//			res.Meta = utils.GetMetaResponse("todolist.acquitision.failed")
	//			return
	//		}
	...
	for i := 0; i < len(req.Label); i++ {
		if req.Body[i] != "" {
			...
			if req.ContentType[i] == "Akuisisi" {
				...
				redisKeyAkuisisi := ottoutils.GetEnv("REDIS_KEY_AKUISISI", "")
				_, err = redis.GetRedisKey(redisKeyAkuisisi + req.Acquitisions.PhoneNumber)
				if err != nil {
					res.Meta = utils.GetMetaResponse("todolist.acquitision.failed")
					return
				}
	...
	if req.Reason == "" {
		...
		if req.TodolistCategoryId == "6" {
			err := postgres.UpdateStatusSuccessMerchantNewRec(req.IdMerchant)
			if err != nil {
				res.Meta = utils.GetMetaResponse("todolist.failed.update.status.db")
				return
			}
	...
	err := postgres.PostHistoryTodolist(request2)
	if err != nil {
		res.Meta = utils.GetMetaResponse("todolist.failed.update.status.db")
		return
	}
